...
echo [2/7] 필요한 패키지 설치/업데이트 중...
python -m pip install --upgrade pip
pip install -r scanner\requirements.txt
if errorlevel 1 goto :error

echo [3/7] 기존 A/B/C 업비트 KRW 코인 스캔 실행 중...
python scanner\box_screener.py
if errorlevel 1 goto :error

echo [4/7] 급등 전 D형 재탈환·압축 스캔 실행 중...
...
echo [5/7] 급락 후 E형 0.382 기술적 반등 스캔 실행 중...
python scanner\technical_rebound.py
if errorlevel 1 goto :error

echo [6/7] 상위 후보 차트와 현재 결과를 저장 중...
...
echo [7/7] A/B/C/D/E Lite 화면 생성 중...
