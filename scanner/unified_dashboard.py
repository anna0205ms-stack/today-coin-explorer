...
def bitcoin_tradingview_widget():
    ...
    config = json.dumps({
        "autosize": True, "symbol": "UPBIT:BTCKRW", "interval": "240", "timezone": "Asia/Seoul",
        "theme": "dark", "style": "1", "locale": "kr", "allow_symbol_change": False,
        "calendar": False, "support_host": "https://www.tradingview.com",
    }, ensure_ascii=False).replace("</", "<\\/")
...
def dashboard_page(snapshot, watch, btc, market_data, regime):
    ...
    intro=page_intro("메인 대시보드","BTC와 시장 자금 흐름을 먼저 보고 오늘 알트코인을 매매해도 되는지 판단하는 곳","① BTC 일봉·4시간봉 → ② 시장 M단계 → ③ BTC.D·TOTAL2·OTHERS → ④ 최종 후보")
    stage=fmt(regime.get("stage") or "M?");name=fmt(regime.get("name") or "시장 데이터 확인")
    btc_location=box_location_text(box)
    body=intro+f'''<section class="panel btc-live-card"><div class="btc-live-head"><div><h2>BTC · 업비트 실시간 차트</h2><div class="sub">차트 왼쪽 위 시간봉 메뉴에서 <b>1일</b> 또는 <b>4시간</b>을 선택해서 봐.</div></div><div><b>₩{fmt(btc.get("price"))}</b><div class="sub">{str(btc.get('basis',{}).get('four_hour_end','-')).replace('T',' ')} 마감봉 반영</div></div></div><div class="btc-summary-grid"><div class="btc-summary-item"><span>일봉 해석</span><b>{fmt(btc.get("daily_state"))}</b></div><div class="btc-summary-item"><span>4시간봉 해석</span><b>{fmt(btc.get("four_hour_state"))}</b></div><div class="btc-summary-item"><span>현재 가격 위치</span><b>{fmt(btc_location)}</b></div><div class="btc-summary-item"><span>‘박스’란?</span><b>최근 30일 주요 저점~고점 가격 범위</b></div></div>{bitcoin_tradingview_widget()}</section>
