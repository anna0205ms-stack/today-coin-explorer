...
def nav(active="dashboard"):
    ...
    scan_menu = f'''<details class="nav-drop"><summary class="{"active" if scan_active else ""}">오늘의 전체 스캔 <span>▾</span></summary><div class="nav-drop-menu"><a class="{"active" if active == "today" else ""}" href="scan.html">전체 보기</a>{''.join(f'<a class="{"active" if active == f"type_{key.lower()}" else ""}" href="type_{key.lower()}.html">{key}형</a>' for key in "ABCDEF")}</div></details>'''
...
def shell(title, body, basis, active="dashboard"):
    ...
    nav_behavior='''<script>document.addEventListener("DOMContentLoaded",()=>{const menus=[...document.querySelectorAll("details.nav-drop")];menus.forEach(menu=>{menu.removeAttribute("open");menu.addEventListener("toggle",()=>{if(menu.open)menus.forEach(other=>{if(other!==menu)other.removeAttribute("open")})});menu.querySelectorAll(".nav-drop-menu a").forEach(link=>link.addEventListener("click",()=>menu.removeAttribute("open")))});document.addEventListener("click",event=>{if(!event.target.closest("details.nav-drop"))menus.forEach(menu=>menu.removeAttribute("open"))})})</script>'''
