...
def type_page(key, snapshot):
    ...
    for i,r in enumerate(rows):
        ...
        stage_line = f'<br><b>D형 생애주기</b> · {fmt(r.get("d_stage"))} {fmt(r.get("d_stage_label"))}<br><b>단계 근거</b> · {fmt(r.get("d_stage_reason"))}' if key == "D" else f'<br><b>차트 진행상태</b> · {fmt(r.get("f_stage"))} {fmt(r.get("f_stage_label"))}<br><b>글로벌 매물대</b> · {fmt((r.get("global_zone") or {}).get("lower"))} ~ {fmt((r.get("global_zone") or {}).get("upper"))} USDT' if key == "F" else ""
        ...
        stage_badge = f'<span class="badge">{fmt(r.get("d_stage"))} · {fmt(r.get("d_stage_label"))}</span><br>' if key == "D" else f'<span class="badge">{fmt(r.get("f_stage"))} · {fmt(r.get("f_stage_label"))}</span><br>' if key == "F" else ""
