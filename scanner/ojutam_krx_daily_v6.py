...
def scan_page(universe,buckets,date):
    ...
    data={'market_date':date,'generated_at':now,'universe_count':app.LIQUIDITY_STATS.get('eligible',len(universe)),'liquidity_filter':app.LIQUIDITY_STATS,'counts':{k:len(buckets[k]) for k in app.LETTERS},'candidates':[r for k in app.LETTERS for r in buckets[k]]}
