...
def inject_page(path: Path, asset: str) -> bool:
    ...
    is_watchlist = path.name == "watchlist.html"
    if MARK in text:
        return False
    if "scan_v6.js" not in text and not is_watchlist:
        return False
    if is_watchlist:
        text = text.replace("관심종목 추적", "내 매매 추적 목록")
        text = text.replace("별표한 차트를 다시 모아보는 곳.", "별표 당시 계획과 현재 분석을 비교하고, 실제 매수 여부까지 관리하는 곳.")
    tag = f'<!-- {MARK} --><script src="{asset}?v=20260908-r12-watchfix"></script>'
