...
def calculate_proxy(global_payload: dict, coins: list[dict]) -> dict:
    """CoinGecko 응답을 오코탐 시장지표 형식으로 변환한다."""
    data = global_payload.get("data", global_payload)
    total = float((data.get("total_market_cap") or {}).get("usd") or 0)
    total_change = data.get("market_cap_change_percentage_24h_usd")
    ranked = sorted(
    ...
    if not total or not btc:
        raise ValueError("CoinGecko 전체 시총 또는 BTC 시총이 없습니다")

    btc_cap = float(btc["market_cap"])
    total2 = max(total - btc_cap, 0)
    top10 = ranked[:10]
    others = max(total - sum(float(row["market_cap"]) for row in top10), 0)
    previous_total = _previous(total, float(total_change) if total_change is not None else None)
    previous_caps = {
        row.get("id"): _previous(
            float(row["market_cap"]),
            float(row["market_cap_change_percentage_24h"])
            if row.get("market_cap_change_percentage_24h") is not None
            else None,
        )
        for row in ranked
    }
    ...
    previous_top10 = [previous_caps.get(row.get("id")) for row in top10]
    previous_others = (
        previous_total - sum(previous_top10)
        if previous_total is not None and all(value is not None for value in previous_top10)
        else None
    )
    ...
    return {
        ...
        "definitions": {
            "btc_d": "BTC 시총 / CoinGecko 전체 시총",
            "total2": "CoinGecko 전체 시총 - BTC 시총",
            "others": "CoinGecko 전체 시총 - 현재 시총 상위 10개 합계",
        },
        "btc": {
...
def _add_snapshot(result: dict, now: datetime) -> dict:
    ...
    prior = [row for row in records if datetime.fromisoformat(row["slot"]) <= current_at - timedelta(hours=4)]
    if prior:
        last = prior[-1]
...
def collect(now: datetime | None = None) -> dict:
    ...
    result = calculate_proxy(global_payload, coins if isinstance(coins, list) else [])
