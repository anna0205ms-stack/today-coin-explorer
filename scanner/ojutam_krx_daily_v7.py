...
def patch_mobile_chart_script():
    ...
    text=text.replace("rightPriceScale:{borderColor:'#29404d',autoScale:true}", "rightPriceScale:{borderColor:'#29404d',autoScale:true,visible:!mobile}")
    text=text.replace("timeScale:{borderColor:'#29404d',rightOffset:8,barSpacing:7,minBarSpacing:2}", "timeScale:{borderColor:'#29404d',rightOffset:mobile?1:8,barSpacing:mobile?5:7,minBarSpacing:2}")
    text=text.replace("priceLineVisible:true,lastValueVisible:true", "priceLineVisible:!mobile,lastValueVisible:!mobile")
    text=text.replace("axisLabelVisible:true,title:k.replaceAll('_',' ')", "axisLabelVisible:!mobile,title:k.replaceAll('_',' ')")
    text=text.replace("axisLabelVisible:true,title})", "axisLabelVisible:!mobile,title})")
    text=text.replace("chart.timeScale().fitContent();ro=new ResizeObserver(()=>chart?.resize(el.clientWidth,el.clientHeight));", "chart.timeScale().fitContent();requestAnimationFrame(()=>chart?.resize(Math.floor(el.getBoundingClientRect().width),el.clientHeight));ro=new ResizeObserver(()=>chart?.resize(Math.floor(el.getBoundingClientRect().width),el.clientHeight));")
    p.write_text(text,encoding='utf-8')
