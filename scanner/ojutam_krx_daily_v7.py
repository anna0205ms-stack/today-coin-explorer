...
def scan_all_candidates(universe, frames):
    """유형별 30개 절단을 제거하고 실제 탐지된 전체 후보 수를 유지한다."""
    ...
    for _, r in universe.iterrows():
        ...
        profile = app.liquidity_profile(frames.get(code), str(r.Market))
        if not profile:
            stats["excluded"] += 1
            continue
        stats["eligible"] += 1
        stats["strong" if profile["tier"] == "충분" else "normal"] += 1
        for item in app.analyze_one(code, str(r.Name), str(r.Market), frames.get(code)):
    ...
    print("liquidity", stats, "pattern totals", {k: len(buckets[k]) for k in app.LETTERS})
    return buckets


# KRX에서는 실제 전체 탐지 개수를 그대로 카드/탭에 표시한다.
app.scan = scan_all_candidates
...
def patch_all_pages_release_footer():
    """OJUTAM의 모든 생성 HTML 하단에 발행자/저작권/버전 타임라인을 표시한다."""
    root = Path('outputs/ojutam')
    css = '''<style id="ojutam-release-footer-style">
.ojutam-release-footer{max-width:1640px;margin:48px auto 20px;padding:18px 20px;border-top:1px solid #17392a;color:#789489;font-size:11px;line-height:1.55}.ojutam-release-main{display:flex;gap:10px 18px;align-items:center;justify-content:space-between;flex-wrap:wrap}.ojutam-release-main b{color:#b7d5c8;letter-spacing:.04em}.ojutam-release-copy{color:#5f786d}.ojutam-release-history{margin-top:10px;border:1px solid #17392a;border-radius:10px;background:#06100b;padding:8px 11px}.ojutam-release-history summary{cursor:pointer;color:#92ad9f;font-weight:700}.ojutam-release-history ul{margin:8px 0 2px;padding-left:18px}.ojutam-release-history li{margin:3px 0}.ojutam-release-ver{color:#72e7b1;font-weight:800}@media(max-width:760px){.ojutam-release-footer{margin-top:28px;padding:14px 12px;font-size:10px}.ojutam-release-main{display:block}.ojutam-release-copy{margin-top:5px}}
</style>'''
    footer = '''<footer class="ojutam-release-footer" data-release="v4.1"><div class="ojutam-release-main"><div><b>Published by JIWON · OJUTAM</b> &nbsp; <span class="ojutam-release-ver">v4.1</span></div><div class="ojutam-release-copy">© 2026 JIWON. All Rights Reserved. Unauthorized copying, reproduction or redistribution prohibited.</div></div><details class="ojutam-release-history"><summary>Version history · 업데이트 기록</summary><ul><li><span class="ojutam-release-ver">v4.1 · 2026-09-08</span> — 유형별 실제 전체 탐지 수 표시 · 전 페이지 발행/저작권/버전 타임라인 적용</li><li><span class="ojutam-release-ver">v4.0 · 2026-09-07</span> — 현재 위치 중심 우선순위 · 평균진입가 · 구조 손절 · 1차/2차 목표 적용</li></ul></details></footer>'''
