...
def patch_mobile_chart_width():
    p=Path('outputs/ojutam/scan_v6.js')
    text=p.read_text(encoding='utf-8')
    text=text.replace("if(!r)return;const el=document.getElementById(`chart-${code}-${type}`);", "if(!r)return;const mobileChart=window.matchMedia('(max-width:760px)').matches;const el=document.getElementById(`chart-${code}-${type}`);")
    text=text.replace("rightPriceScale:{borderColor:'#29404d',autoScale:true}", "rightPriceScale:{borderColor:'#29404d',autoScale:true,minimumWidth:mobileChart?42:70}")
    text=text.replace("timeScale:{borderColor:'#29404d',rightOffset:8,barSpacing:7,minBarSpacing:2}", "timeScale:{borderColor:'#29404d',rightOffset:mobileChart?2:8,barSpacing:mobileChart?5:7,minBarSpacing:2}")
    text=text.replace("axisLabelVisible:true,title:k.replaceAll('_',' ')", "axisLabelVisible:!mobileChart,title:mobileChart?'':k.replaceAll('_',' ')")
    text=text.replace("axisLabelVisible:true,title})", "axisLabelVisible:!mobileChart,title:mobileChart?'':title})")
    text=text.replace("priceLineVisible:true,lastValueVisible:true", "priceLineVisible:true,lastValueVisible:!mobileChart")
    p.write_text(text,encoding='utf-8')
